...
## 2026-08-27 — `MessagingApi` is derived from the package, not transcribed (departs from `@/lib/analytics` and `@/lib/crash-reporting`)
**Chose:** `Pick<typeof FirebaseMessaging, 'getMessaging' | …>` over the package's module type, naming the eight modular exports this wrapper calls
**Over:** (a) the hand-written member-by-member contract the two sibling telemetry wrappers use, which is what this module shipped with; (b) `Pick<typeof import('@react-native-firebase/messaging'), …>` written inline, as review suggested
**Why:** A transcribed contract cannot fail: `loadApi()` casts an untyped `require()` result onto it, so whatever it claims is what the compiler believes, and an upstream rename or signature change lands at runtime. It had already gone lossy — not wrong, and still assignable to the real module, but both permission calls were widened to `Promise<number>` where the package returns `AuthorizationStatus` (`-1 | 0 | 1 | 2 | 3`), and `getToken`'s `options?` parameter was missing, narrowing what this module could pass. Losing precision silently, without ever failing to compile, is the failure mode: it is invisible until someone relies on a guarantee the transcription no longer carries. `Pick` keeps the one thing the local contract was written for, a single place naming the surface depended on, and binds it to the package: renaming a member to a non-export is now `TS2344` at the type itself (verified by probe). The sibling convention is not being broken so much as declined where its cause does not apply — `@/lib/analytics` documents a real blocker, since that package declares `logEvent` as 27 event-name overloads plus one generic form and a `Pick` of it will not take this app's single generic call shape. Messaging has no overloads or generics on any of the eight; every one is a plain signature. (b) is the same type but trips `ts/consistent-type-imports`, which `eslint.config.mjs` sets with `disallowTypeAnnotations: true`; a `import type * as` namespace import is the lint-clean spelling of it.
**Deliberately not done:** `@/lib/crash-reporting` is derivable too — its six exports are plain signatures — and should follow. Changing it here would be scope creep on a push-notifications PR, and it has its own tests and spec to update.
**Trade-off:** This module's build now moves with the package's export names, so a rename upstream breaks compilation even where nothing here behaves differently, and an SDK bump can turn into type work before the app runs. That is the trade, not a side effect — a silently stale transcription is the failure worth more. It also inherits upstream's `@deprecated` markers on `requestPermission` and `hasPermission`; no rule in this repo errors on those today, so they read as the warning they are.
