...
- **No OTA updates, so no `channel` keys** — this app does not ship `expo-updates`. Build profiles select builds by `--profile`; a `channel` in `eas.json` would be inert. Do not add one without also adding `expo-updates`, an `updates.url` and a `runtimeVersion` policy, which is a product decision. See **Decisions** below.
...
**Trade-off:** Should this app adopt EAS Update later, the channels must be re-added — alongside `expo-updates`, an `updates.url`, and a `runtimeVersion` policy, none of which the deleted keys supplied and which are the parts that actually take thought. `app.config.ts` is dynamic, so `eas update:configure` reports those values rather than writing them: they have to be added by hand. Until then `eas.json` no longer implies an OTA setup that does not exist.
