...
Animations are applied to the icons using `Animated.View` with `useAnimatedStyle` from `react-native-reanimated`. These animations change the appearance of the icons based on their checked state.
